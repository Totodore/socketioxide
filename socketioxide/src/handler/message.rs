...
pub(crate) trait ErasedMessageHandler: Send + Sync + 'static {
    fn call(&self, s: Arc<Socket>, v: Value, p: Vec<Vec<u8>>, ack_id: Option<i64>);
}
...
{
    fn call(&self, _: Arc<Socket>, _: Value, _: Vec<Vec<u8>>, _: Option<i64>) {
        let fut = (self.clone())();
        tokio::spawn(fut);
    }
}
...
macro_rules! impl_async_handler {
    (
        ...
        {
            fn call(&self, s: Arc<Socket>, mut v: Value, mut p: Vec<Vec<u8>>, ack_id: Option<i64>) {
                $(
