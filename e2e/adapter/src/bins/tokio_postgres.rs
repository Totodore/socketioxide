...
use socketioxide_postgres::{
    PostgresAdapterConfig, PostgresAdapterCtr, TokioPostgresAdapter,
    drivers::tokio_postgres::tokio_postgres_client::{Config, NoTls},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pg_config = Config::from_str("postgres://socketio:socketio@localhost:5432/socketio")?;

    let adapter =
        PostgresAdapterCtr::new_with_tokio_postgres_config(pg_config, NoTls, config).await?;
