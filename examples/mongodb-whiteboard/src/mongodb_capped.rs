...
use socketioxide_mongodb::{
    drivers::mongodb::mongodb_client as mongodb, MessageExpirationStrategy, MongoDbAdapterConfig,
};
...
const DB: &str = "whiteboard-capped";
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = MongoDbAdapterConfig::new()
        .with_expiration_strategy(MessageExpirationStrategy::CappedCollection(8_000_000)); // 8MB capped collection
    let adapter = MongoDbAdapterCtr::new_with_mongodb_config(db, config).await?;
