...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    io.ns("/", move |s: Arc<Socket>| async move {
        s.on(
            "new message",
            |s: Arc<Socket>, msg: String, _, _| async move {
        ...
        s.on(
            "add user",
            move |s: Arc<Socket>, username: String, _, _| async move {
        ...
        s.on("typing", |s: Arc<Socket>, _: [(); 0], _, _| async move {
            let username = s.extensions.get::<Username>().unwrap().clone();
            s.broadcast()
        ...
        s.on(
            "stop typing",
            |s: Arc<Socket>, _: [(); 0], _, _| async move {
                let username = s.extensions.get::<Username>().unwrap().clone();
                s.broadcast()
